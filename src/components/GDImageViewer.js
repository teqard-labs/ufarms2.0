...
function GDImageViewer(data) {
  ...
  function ModalView(props) {
    return (
      <div>
        <div id="modal-container" className="modal top-[20vw] md:top-[7vw] px-3 w-full h-full overflow-y-hidden ">
        ...
        {imgIds &&
          imgIds.map((item, i) => {
            ...
            if (checkFormat(item.title)) {
              ...
              return (
                <div key={item.id}> {/* Add a unique key prop */}
                  {renderMain(className, id, exclude, href, target, item, i)}
                </div>
              );
            } else {
